...
if ($do == 'display') {
	...
	foreach($own_account_modules['modules'] as $account_module_name => &$account_module_info) {
		if ($account_module_info['role'] == 'clerk') {
						$user_module_permission_info = table('users_permission')->getUserPermissionByType($_W['uid'], $account_module_info['uniacid'], $account_module_info['module_name']);
