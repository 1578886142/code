...
class Common_EweiShopV2Model 
{
	...
	public function AliPayBarcode($params, $config) 
	{
		...
		return error($result['alipay_trade_pay_response']['code'], $result['alipay_trade_pay_response']['msg'] . ':' . $result['alipay_trade_pay_response']['sub_msg']);
	}
	...
	public function AliPayQuery($out_trade_no, $config) 
	{
		...
		if (!(empty($result['alipay_trade_query_response']['trade_status'])) && ($result['alipay_trade_query_response']['trade_status'] == 'TRADE_CLOSED')) 
		{
			return error($result['alipay_trade_query_response']['code'], '该订单已经关闭或者已经退款');
		}
		return error($result['alipay_trade_query_response']['code'], $result['alipay_trade_query_response']['msg'] . ':' . $result['alipay_trade_query_response']['sub_msg']);
	}
	public function AliPayRefundQuery($out_trade_no, $config) 
	{
		...
		return error($result['alipay_trade_fastpay_refund_query_response']['code'], $result['alipay_trade_fastpay_refund_query_response']['msg'] . ':' . $result['alipay_trade_fastpay_refund_query_response']['sub_msg']);
	}
