	...
	function tpl_selector_new($name, $options = array()) 
	{
		...
		foreach ($options['items'] as $item ) 
		{
			...
			else if ($options['type'] == 'product') 
			{
				...
				if ($item['optiontitle']) 
				{
					$html .= '<a class=\'btn btn-default btn-sm\' data-toggle=\'ajaxModal\' href=\'' . webUrl('sale/package/hasoption', array('goodsid' => $item['goodsid'], 'pid' => $item['pid'])) . '\' id=\'optiontitle' . $item['goodsid'] . '\'>' . $optiontitle . '</a>' . "\n" . '                            <input type=\'hidden\' id=\'packagegoods' . $item['goodsid'] . '\' value=\'' . $item['option'] . '\' name=\'packagegoods[' . $item['goodsid'] . ']\'>';
