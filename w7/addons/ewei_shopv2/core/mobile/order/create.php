	...
	public function main() 
	{
		...
		if (!($packageid)) 
		{
			...
			foreach ($goods as &$g ) 
			{
				...
				if (!(empty($g['virtual'])) || ($g['type'] == 2) || ($g['type'] == 20)) 
				{
					$isvirtual = true;
					if ($g['virtualsend']) 
					{
			...
			$createInfo = array('id' => $id, 'gdid' => intval($_GPC['gdid']), 'fromcart' => $fromcart, 'addressid' => (!empty($address) && !$isverify && !$isvirtual ? $address['id'] : 0), 'storeid' => 0, 'couponcount' => $couponcount, 'isvirtual' => $isvirtual, 'isverify' => $isverify, 'goods' => $goodsdata, 'merchs' => $merchs, 'orderdiyformid' => $orderdiyformid, 'token' => $token, 'giftid' => $giftid, 'mustbind' => $mustbind);
	...
	public function caculate() 
	{
		...
		if (is_array($goodsarr)) 
		{
			...
			foreach ($goodsarr as &$g ) 
			{
				...
				if (!(empty($data['virtual'])) || ($data['type'] == 2) || ($data['type'] == 20)) 
				{
					$isvirtual = true;
				}
				$g['taskdiscountprice'] = $prices['taskdiscountprice'];
