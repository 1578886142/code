...
class Apply_EweiShopV2Page extends PluginWebPage 
{
	...
	public function detail() 
	{
		...
		extract($applyData);
		$repurchase = ((isset($apply['repurchase']) ? (double) $apply['repurchase'] : 0));
		$pay = max($realmoney - $repurchase, 0);
		include $this->template();
	}
	...
	public function refuse() 
	{
		...
		pdo_delete('ewei_shop_commission_repurchase', array('applyid' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function pay($params = array(), $mine = array()) 
	{
		...
		if ($apply['type'] < 2) 
		{
			...
			if ((0 < $repurchase) && ($pay <= 0)) 
			{
				m('finance')->pay($member['openid'], 0, $repurchase, $apply['applyno'], '复消' . $set['texts']['commission'] . '打款致余额');
			}
		}
		foreach ($list as $row ) 
		{
		...
		if (p('author')) 
		{
			p('author')->upgradeLevelByCommissionOK($member['openid']);
		}
