...
class Level_EweiShopV2Page extends PluginWebPage 
{
	...
	protected function post() 
	{
		...
		if ($id == 'default') 
		{
			$level = array('id' => 'default', 'levelname' => (empty($set['levelname']) ? '默认等级' : $set['levelname']), 'commission1' => $set['commission1'], 'commission2' => $set['commission2'], 'commission3' => $set['commission3'], 'withdraw' => (double) $set['withdraw_default'], 'repurchase' => (double) $set['repurchase_default']);
		}
		else 
		{
		...
		if ($_W['ispost']) 
		{
			...
			if (!empty($id)) 
			{
				if ($id == 'default') 
				{
					...
					$set['withdraw_default'] = $data['withdraw'];
					$set['repurchase_default'] = $data['repurchase'];
					$this->updateSet($set);
